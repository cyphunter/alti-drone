          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-3 p-4 sm:p-5">
              <div className="min-w-0 translate-y-1 transition-transform duration-500 group-hover:translate-y-0">
                {g.service ? (
                  <p className="text-[10px] uppercase tracking-[0.18em] text-accent-400">
                    {g.service.replace(/-/g, " ")}
                  </p>
                ) : null}
                {g.caption ? (
                  <p className="mt-1 line-clamp-2 font-display text-sm leading-snug text-paper sm:text-base">
                    {g.caption}
                  </p>
                ) : null}
              </div>
              <span
                aria-hidden
                className="hidden h-9 w-9 shrink-0 items-center justify-center rounded-full bg-paper/10 text-paper ring-1 ring-paper/30 backdrop-blur-md transition-all duration-500 group-hover:bg-accent-500 group-hover:text-ocean-900 sm:inline-flex"
              >
                <Maximize2 size={14} />
              </span>
            </div>

            {/* Hairline doré inferior on hover */}
            <span
              aria-hidden
